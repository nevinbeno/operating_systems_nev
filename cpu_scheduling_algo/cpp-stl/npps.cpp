...
bool compare_priority(const struct process* a, const process* b)
{
...
bool npps(vector<struct process>& p)
{
    int n_proc = p.size();
    if (n_proc < 1)
        return false;
    ...
    while(completed < n_proc)
    {
        ...
        {
            sort(q.begin(), q.end(), compare_priority);
...
void display(vector<struct process>& p)
{
    ...
    cout<<"== Non-Preemptive (Lower number higher priority)Priority Scheduling =="<<endl;
    cout<<"PID\tAT\tBT\tPriority\tCT\tTAT\tWT\tResponse Time"<<endl;
    for (int i = 0; i < total_elements; i++)
        cout<<p[i].id<<"\t"<<p[i].at<<"\t"<<p[i].bt<<"\t"<<p[i].priority<<"\t\t"<<p[i].ct<<"\t"<<p[i].tat<<"\t"<<p[i].wt<<"\t"<<p[i].rt<<endl;
    return;
}
